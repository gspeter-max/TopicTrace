...
class TestWebSearchEdgeCases:
    ...
    def test_api_rate_limit_returns_error(self, MockClient):
        ...
        results = asyncio.run(
            web_search.ainvoke({"query": "test api rate limit unique"})
        )
...
class TestWebFetchEdgeCases:
    ...
    def test_empty_url_returns_error(self):
        ...
        results = asyncio.run(
            web_fetch.ainvoke({"url": "", "query": "test fetch empty"})
        )
    ...
    def test_whitespace_url_returns_error(self):
        ...
        results = asyncio.run(
            web_fetch.ainvoke({"url": "   ", "query": "test fetch whitespace"})
        )
        assert results[0]["status"] == "error"
    ...
    def test_404_returns_error(self, MockClient, mock_load, mock_save, mock_llm):
        ...
        results = asyncio.run(
            web_fetch.ainvoke(
                {"url": "https://example.com/notfound", "query": "test 404"}
            )
        )
    ...
    def test_500_returns_error(self, MockClient, mock_load, mock_save, mock_llm):
        ...
        results = asyncio.run(
            web_fetch.ainvoke({"url": "https://example.com/error", "query": "test 500"})
        )
    ...
    def test_network_error_returns_error(
        self, MockClient, mock_load, mock_save, mock_llm
    ):
        """Network error should return error dict."""
        MockClient.return_value.__aenter__.return_value.get.side_effect = (
            ConnectionError("Network unreachable")
        )
        from topictrace.tools.web_fetch import web_fetch

        results = asyncio.run(
            web_fetch.ainvoke({"url": "https://example.com", "query": "test network"})
        )
        assert results[0]["status"] == "error"
    ...
    def test_timeout_returns_error(self, MockClient, mock_load, mock_save, mock_llm):
        ...
        MockClient.return_value.__aenter__.return_value.get.side_effect = TimeoutError(
            "Request timed out"
        )
        from topictrace.tools.web_fetch import web_fetch

        results = asyncio.run(
            web_fetch.ainvoke({"url": "https://example.com", "query": "test timeout"})
        )
        assert results[0]["status"] == "error"
    ...
    @patch("topictrace.tools.web_fetch.load_from_cache", return_value=None)
    @patch("topictrace.tools.web_fetch.httpx.AsyncClient")
    def test_successful_fetch_calls_llm(
        self, MockClient, mock_load, mock_save, mock_llm
    ):
        ...
        results = asyncio.run(
            web_fetch.ainvoke({"url": "https://example.com", "query": "test"})
        )
    ...
    def test_cache_hit_skips_fetch(self, mock_load):
        ...
        results = asyncio.run(
            web_fetch.ainvoke({"url": "https://example.com", "query": "test cache hit"})
        )
    ...
    def test_jina_url_construction(self, MockClient, mock_load, mock_save, mock_llm):
        ...
        asyncio.run(
            web_fetch.ainvoke({"url": "https://example.com/page", "query": "test jina"})
        )
    ...
    def test_list_of_urls(self, MockClient, mock_load, mock_save, mock_llm):
        ...
        results = asyncio.run(
            web_fetch.ainvoke(
                {"url": ["https://a.com", "https://b.com"], "query": "test list"}
            )
        )
...
class TestAuthMiddleware:
    ...
    def test_valid_key_allows_request(
        ...
        mock_conn.cursor.return_value.__enter__ = MagicMock(return_value=mock_cursor)
        mock_conn.cursor.return_value.__exit__ = MagicMock(return_value=False)
        mock_middleware_pool.connection.return_value.__enter__ = MagicMock(
            return_value=mock_conn
        )
        mock_middleware_pool.connection.return_value.__exit__ = MagicMock(
            return_value=False
        )
